...
function ensureEnvFromCiSnippet() {
  // Assumes CI or local env has created .env like in the workflow. No-op here.
  if (!fs.existsSync('.env')) {
    process.stderr.write('Warning: .env not found. Ensure OPENAPI env vars are set as in CI.\n');
  }
}
...
function main() {
  ...
  const progress = fs.existsSync(progressPath) ? readJSON(progressPath) : { mode, lastProcessed: null, endpoints: {} };
  const config = fs.existsSync(configPath) ? readJSON(configPath) : { defaultMode: mode, remote };

  // Discover endpoints
  const collect = spawnSync('node', ['scripts/openapi/collect-endpoints.js', mode], { encoding: 'utf8' });
  if (collect.status !== 0) {
  ...
  // Pick next pending
  const next = allEndpoints.find(ep => progress.endpoints[ep.path]?.status === 'pending');
  if (!next) {
