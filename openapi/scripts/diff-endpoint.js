...
function diffParameters(codeParams = [], loadParams = [], options = {}) {
	...
	if (
		!diffs.missingInLoaded.length &&
		!diffs.missingInCode.length &&
...
function diffRequestBody(codeBody = {}, loadBody = {}) {
	...
	for (const t of allTypes) {
		...
		else if (!deepEqual(c, l))
			result.schemaDiffs.push({ contentType: t, code: c, loaded: l });
	}
	if (
		!result.missingInLoaded.length &&
		!result.missingInCode.length &&
		!result.schemaDiffs.length
	)
		return null;
	return result;
}
...
function diffResponses(codeResponses = {}, loadResponses = {}) {
	...
	const codes = new Set([
		...Object.keys(codeResponses || {}),
		...Object.keys(loadResponses || {}),
	]);
	...
	for (const sc of codes) {
		...
		if (c && !l) {
			result.missingInLoaded.push(sc);
			continue;
		}
		if (!c && l) {
			result.missingInCode.push(sc);
			continue;
		}
		...
		for (const t of cTypes) {
			...
			else if (cSchema && !lSchema)
				result.contentDiffs.push({
					status: sc,
					contentType: t,
					missingInLoaded: true,
				});
			else if (!deepEqual(cSchema, lSchema))
				result.contentDiffs.push({
					status: sc,
					contentType: t,
					code: cSchema,
...
function main() {
	...
	if (!logicalPath) {
		console.error(
			"Usage: node openapi/scripts/diff-endpoint.js <store|admin> <logicalPath e.g. /cms/{id}> [method]",
		);
		process.exitCode = 2;
		return;
	}
	...
	const schemaFile = path.join(
		rootDir,
		`api-types/${mode === "admin" ? "admin" : "store"}ApiSchema.json`,
	);
	if (!fs.existsSync(schemaFile)) {
		// ensure schema is loaded
		run(
			apiGenBin,
			["loadSchema", `--apiType=${mode === "admin" ? "admin" : "store"}`],
			{ env: envVars },
		);
	}
	...
	const loadedPathDef = findLoadedPathDef(
		loaded.paths || {},
		runtimePath,
		logicalPath,
	);
	...
	for (const m of methodsToCheck) {
		...
		if (!codeDef || !loadDef) {
			console.log(
				`Method ${m.toUpperCase()}: present in ${codeDef ? "code" : "-"}/${loadDef ? "loaded" : "-"}`,
			);
			hasDiff = true;
			continue;
		}
		const codeHasListingGroup =
			Array.isArray(codeDef.parameters) &&
			codeDef.parameters.some(
				(p) => p && p["x-parameter-group"] === "productListingCriteria",
			);
		const p = diffParameters(codeDef.parameters, loadDef.parameters, {
			codeHasListingGroup,
		});
		...
		const pIsMeaningful =
			p &&
			(p.missingInLoaded.length || p.missingInCode.length || p.changed.length);
	...
	if (!hasDiff) {
		console.log(
			`No differences for ${runtimePath}${method ? ` ${method.toUpperCase()}` : ""}`,
		);
	} else {
		process.exitCode = 1;
	}
